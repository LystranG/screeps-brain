...
describe("official-sim-style bootstrap", function () {
  ...
  it("surfaces manually placed spawn energy waits as waiting instead of stuck queued", async () => {
    helper = await createSimReadyScenario({ roomName: "W1N1" });
    const activeHelper = helper;

    await activeHelper.setSpawnEnergy("W1N1", 0);
    ...
    await activeHelper.tickUntil(
      async () => {
        ...
        return queue.some(request => request.lastError === "-6");
      },
      50,
      "sim manually placed spawn energy wait"
    );

    const memory = await activeHelper.readMemory();
    const waitingRequests = memory.colonies.W1N1.spawnQueue.filter(request => request.lastError === "-6");
    const spawnStatus = await activeHelper.runCommand("cmd.spawn.status()");
    const spawnQueue = await activeHelper.runCommand("cmd.spawn.queue()");

    assert.isAtLeast(waitingRequests.length, 1);
    assert.isTrue(
      waitingRequests.every(request => request.status === "waiting"),
      JSON.stringify(waitingRequests, null, 2)
    );
    assertCommandIncludes(spawnStatus, ["waiting="]);
    assertCommandIncludes(spawnQueue, ["status=waiting", "lastError=-6"]);
  });

  it("recovers waiting manually placed spawn requests after energy becomes available", async () => {
    helper = await createSimReadyScenario({ roomName: "W1N1" });
    const activeHelper = helper;
    ...
    await activeHelper.tickUntil(
      async () => {
        ...
        return queue.some(request => request.status === "waiting" && request.lastError === "-6");
      },
      50,
      "sim waiting before energy restore"
    );

    await activeHelper.setSpawnEnergy("W1N1", 300);
    await activeHelper.tickUntil(
      async () => {
        const memory = await activeHelper.readMemory();
        const queue = memory.colonies?.W1N1?.spawnQueue ?? [];

        return queue.some(request => request.status === "spawning" || request.status === "spawned");
      },
      250,
      "sim waiting request recovery after energy restore"
    );
    ...
    assert.isFalse(
      queue.some(request => request.status === "queued" && request.lastError === "-6"),
      JSON.stringify(queue, null, 2)
    );
    assert.isTrue(queue.some(request => request.status === "spawning" || request.status === "spawned"));
  });

  it("repairs existing queued energy-error requests from prior sim ticks", async () => {
    helper = await createSimReadyScenario({ roomName: "W1N1" });
    const activeHelper = helper;
    ...
    await activeHelper.tickUntil(
      async () => {
        ...
        return memory.colonies?.W1N1?.spawnQueue?.length > 0;
      },
      50,
      "sim queued request before legacy injection"
    );

    const legacyMemory = await activeHelper.readMemory();
    legacyMemory.colonies.W1N1.spawnQueue[0].status = "queued";
    legacyMemory.colonies.W1N1.spawnQueue[0].lastError = "-6";
    await activeHelper.writeMemory(legacyMemory);
    ...
    await activeHelper.tickUntil(
      async () => {
        ...
        return queue.some(request => request.status !== "queued" && request.lastError === "-6");
      },
      50,
      "legacy queued energy error repair"
    );

    const repairedMemory = await activeHelper.readMemory();
    const queue = repairedMemory.colonies.W1N1.spawnQueue;
  ...
  it("repairs existing queued energy-error requests even when sim spawn capacity is unavailable", async () => {
    helper = await createSimReadyScenario({ roomName: "W1N1" });
    const activeHelper = helper;
    ...
    await activeHelper.tickUntil(
      ...
      50,
      "sim queued request before zero-capacity legacy injection"
    );
    ...
    legacyMemory.colonies.W1N1.spawnQueue[0].lastError = "-6";
    await activeHelper.writeMemory(legacyMemory);
    await activeHelper.setSpawnEnergyCapacity("W1N1", 0);
  ...
  it("reconciles visible spawning creeps whose names include a waiting bootstrap request id", async () => {
    helper = await createSimReadyScenario({ roomName: "W1N1" });
    const activeHelper = helper;
    ...
    await activeHelper.tickUntil(
      ...
      50,
      "sim waiting request before visible creep reconciliation"
    );

    const waitingMemory = await activeHelper.readMemory();
    const request = waitingMemory.colonies.W1N1.spawnQueue.find(candidate => candidate.status === "waiting");

    assert.isDefined(request);

    const escapedRequestId = request!.id.replace(/[^A-Za-z0-9_-]/g, "-");
    const visibleCreepName = `bootstrap-${request!.role}-${request!.roomName}-99-${escapedRequestId}`;
    await activeHelper.addWorkerCreep("W1N1", visibleCreepName, { spawning: true });
    ...
    await activeHelper.tickUntil(
      async () => {
        ...
        return queue.some(candidate => candidate.id === request!.id && candidate.status === "spawning");
      },
      50,
      "visible creep request reconciliation"
    );

    const reconciledMemory = await activeHelper.readMemory();
    const reconciledRequest = reconciledMemory.colonies.W1N1.spawnQueue.find(candidate => candidate.id === request!.id);

    assert.equal(reconciledRequest?.status, "spawning");
    assert.equal(reconciledRequest?.spawnName, "SpawnPrimary");
    assert.equal(reconciledRequest?.creepName, visibleCreepName);
    assert.equal(reconciledRequest?.lastError, null);
  });
